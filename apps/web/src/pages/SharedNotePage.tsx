...
import { ApiError, apiRequest, buildWsUrl, formatDate, requestSharedRenderedHtmlPreview, type NotePayload, type Thread, type ThreadAnchor, uploadImage } from '../lib/api';
...
import { getStoredEditorWrapEnabled, getStoredPreviewMode, hasScrolledToNewViewport, renderHistoryBadge, setStoredEditorWrapEnabled, setStoredPreviewMode, type EditorHistoryState, useDocumentTitle } from './page-utils';
...
export function SharedNotePage({ shareId, onToggleTheme }: { shareId: string; onToggleTheme: () => void }) {
  ...
  const renderedPdfPreview = useCallback(() => requestSharedRenderedHtmlPreview(shareId, markdown), [markdown, shareId]);
  const {
    renderedPdfUrl,
    renderedPdfZoom,
    renderedPdfLoading,
