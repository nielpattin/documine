  ...
  for (const span of root.querySelectorAll("span[class]")) {
    for (const cls of span.classList) {
      ...
      if (color) {
        span.setAttribute(
          "style",
          `${span.getAttribute("style") || ""}${color.startsWith("font-") ? color : `color:${color};`}`,
        );
      }
    }
  }
...
function styleClipboardBlock(root: HTMLElement) {
  root.querySelectorAll("h1, h2, h3, h4, h5, h6").forEach((heading) => {
    (heading as HTMLElement).style.cssText =
      "font-family:Times New Roman,serif;color:#000000;font-weight:700;margin:0 0 12pt 0;line-height:1.2;";
  });
  root.querySelectorAll("p, li").forEach((node) => {
    (node as HTMLElement).style.cssText =
      "font-family:Times New Roman,serif;color:#000000;font-size:12pt;line-height:1.45;text-align:left;";
  });
  root.querySelectorAll("ul, ol").forEach((list) => {
    (list as HTMLElement).style.cssText =
      "font-family:Times New Roman,serif;color:#000000;font-size:12pt;line-height:1.45;margin:0 0 12pt 0;";
  });
  ...
  root.querySelectorAll("code").forEach((code) => {
    ...
    (code as HTMLElement).style.cssText =
      "font-family:Consolas,monospace;color:#000000;background-color:#eeeeee;padding:0 2pt;font-size:10pt;";
  });
}
...
export async function copyRenderedPreviewToClipboard(iframe: HTMLIFrameElement | null) {
  ...
  const source =
    previewDocument.getElementById("documine-preview-source") ||
  ...
  contentClone
    .querySelectorAll("script, style, #documine-preview-pages, .documine-preview-pages, #documine-preview-measure")
    .forEach((node) => node.remove());
  contentClone.removeAttribute("id");
  contentClone.removeAttribute("class");
  contentClone.style.cssText =
    "font-family:Times New Roman,serif;color:#000000;font-size:12pt;line-height:1.45;text-align:left;background:#ffffff;";
